...
def list_equipment():
    return jsonify([item for item in EQUIPMENT if is_available(item)])
...
#Added a helper to identify if the equipment is available.
...
def create_booking():
    ...
    if not is_available(equipment):
        return jsonify({"error": "Equipment is unavailable"}), 400
